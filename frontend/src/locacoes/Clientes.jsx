...
const Clientes = () => {
    ...
    const fetchClientes = async () => {
        try {
            const response = await authFetch(import.meta.env.VITE_API_URL + "/clientes/");
            if (response.ok) {
        ...
        <div className="min-h-screen bg-slate-950 text-slate-200 font-sans">
            ...
            <main className="p-4 max-w-4xl mx-auto space-y-6">
                ...
                <div className="relative">
                    ...
                    />
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 absolute left-4 top-3.5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="grid gap-4">
                    {clientesFiltrados.length > 0 ? (
                        clientesFiltrados.map((cliente) => (
                            <div key={cliente.id} className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex justify-between items-center">
                                <div>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        onClick={() => openDeleteModal(cliente.nome, cliente.id)}
                        ...
                        <p className="text-center text-slate-500 py-10">
                            {searchTerm ? "Nenhum cliente corresponde à sua pesquisa." : "Nenhum cliente cadastrado."}
                        </p>
                    )}
                </div>
            </main>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl w-full max-w-md shadow-2xl">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm mb-1 text-slate-400">Nome Completo</label>
                                <input
                                    required
                                    ...
                                    value={formData.nome}
                                    onChange={(e) => setFormData({ ...formData, nome: formatTitleCase(e.target.value) })}
                                />
                            </div>
                            <div>
                                <label className="block text-sm mb-1 text-slate-400">Celular</label>
                                <input
                                    required
                                    type="text"
            ...
            {isDeleteModalOpen && (
                 <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-[60]">
                    <div className="bg-slate-900 border border-red-500/30 p-8 rounded-3xl w-full max-w-sm shadow-2xl text-center">
                        <h2 className="text-xl font-bold text-white mb-2">Remover Cliente</h2>
                        <p className="text-slate-400 mb-8">Deseja remover {clientNameToDelete}?</p>
                        <div className="flex flex-col gap-3">
                            <button onClick={confirmDelete} className="w-full bg-red-600 py-3 rounded-xl font-bold">Sim, remover</button>
                            <button onClick={() => setIsDeleteModalOpen(false)} className="w-full bg-slate-800 py-3 rounded-xl">Cancelar</button>
                        </div>
                    </div>
                 </div>
            )}
